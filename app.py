...
row0_spacer1, row0_1, row0_spacer2, row0_2, row0_spacer3 = st.columns(
    (.1, 2.3, .1, 1.3, .1))
with row0_1:
    st.title('Emissions Analyzer')
    st.markdown('Brought to you by 114&deg;W')
#
...
# to match the sorting we specified.
    ...
    total_df_filtered['Reporting Company Trade Name / Nom commercial de la société déclarante'] = total_df_filtered['Reporting Company Trade Name / Nom commercial de la société déclarante'].apply(lambda x: x.title())
    # Group the data and perform the sum and sorting as before
    total_df_filtered = total_df_filtered.groupby(['Facility Name', 'Reporting Company Trade Name / Nom commercial de la société déclarante'])[
...
def chart2(df_filtered):                                                        # Defines a new function for chart 2 based on data above
    ...
    fig = px.bar(total_df_filtered, x="English Facility NAICS Code Description / Description du code SCIAN de l'installation en anglais", y="Total Emissions (tonnes CO2e) / Émissions totales (tonnes éq. CO2)",
                color='Facility Name')
    # Create a list of top 15 facility names
    top_15_facilities = total_df_filtered.groupby('Facility Name')[co2_column].sum().nlargest(15).index.tolist()

    fig.for_each_trace(lambda trace: trace.update(showlegend=trace.name in top_15_facilities))
...
def chart4(data): # Defines function chart4 that takes in a dataframe called "data" as input
    ...
    data[report_col] = data[report_col].apply(lambda x: x.title())
...
#
with st.sidebar:
    st.subheader('Select one or more First Nations')
    band_chart = st.multiselect("Please select Nations to view Emissions Data:", list(                #Allows users to select multiple bands
...
with row12_1:
    ...
    st.markdown('Contact Alexandria Shrake at 114&deg;W for more detail. Email: Alexandriashrake@gmail.com')
